	...
	public class BlockCreator : MonoBehaviour
	{
		...
		{	
			if (activeHand != null)
			{
				...
				bool downcheck = SteamVR_Input._default.inActions.CreateBlock.GetStateDown(activeHand.handType);
				if (downcheck == true)
				{
					isPressing = true;
					if (isPressing) {
		...
		{
			//TODO: Fill.
			Vector3 CheckPress = GetGridPressPosition();
			if (IsOverlappingOtherBlock (CheckPress) == false) 
			{
				CreateBlock(CheckPress);
			}
		...
		{
			//TODO: Fill.
			GameObject CreatedBlock = Instantiate(blockPrefab);
			CreatedBlock.transform.position = position;
            MeshRenderer BlockMesh = CreatedBlock.GetComponentInChildren<MeshRenderer>();
            BlockMesh.material.SetColor("_BaseColorFactor", color);
            spawnAudioClip
		}
